...
class DiscordBotBattlemetricsBanNotifier(discord.Client):
    ...
    def create_embed_of_ban(self, ban):
        ...
        embedVar = discord.Embed(title="Banned Player (Config 1)", color=0x00ff00)
...
def get_banlist(url, headers):
    ...
    playerNames, banner, banReasons, timeBanned, timeUnbanned, userID, server = ([] for i in range(8))
    ...
    returnList = []
    for l in list(zip(playerNames, banner, banReasons, timeBanned, timeUnbanned, userID, server)):
