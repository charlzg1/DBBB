...
class DiscordBotBattlemetricsBanNotifier(discord.Client):
    ...
    def create_embed_of_ban(self, ban):
        ...
        embedVar = discord.Embed(title="Banned Player (config 3)", color=0x00ff00)
...
def get_banlist(url, headers):
    ...
    playerNames, banner, banReasons, timeBanned, timeUnbanned, userID = ([] for i in range(7))
    ...
    returnList = []
    for l in list(zip(playerNames, banner, banReasons, timeBanned, timeUnbanned, userID)):
