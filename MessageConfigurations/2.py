...
class DiscordBanNotifier(discord.Client):
    ...
    def create_embed_of_ban(self, ban):
        ...
        embedVar = discord.Embed(title="Banned Player (config 2)", color=0x00ff00)
        ...
        embedVar.add_field(name="Ban Notes", value=ban[BanInfo.BAN_NOTE.value], inline=False)
...
def get_banlist(url, headers):
    ...
    returnList = []
    for l in list(zip(playerNames, banner, banReasons, timeBanned, timeUnbanned, userID, banNote, server)):
